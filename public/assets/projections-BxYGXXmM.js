import{b as je,x as Ee,g as Ne,e as C,m as Pe,t as me,p as Ce,j as Be,n as Re,w as Ae,y as Fe,q as Te,o as qe,z as ze,A as Ue}from"./index-ByNsgN5U.js";import{p as He}from"./playerCard-CxaPKWBq.js";import{b as De,r as ve,a as ye}from"./relevance-C3SYLMeE.js";let k=[],f=new Map,u="",l=1,i=!0,_="ALL",Z=new Set,b=!1,S=null;const J=50,We={QB:1.55,RB:1.07,WR:1.12,TE:.88,K:.85,DEF:.75};function U(o,s){const d=We[(o||"UNK").toUpperCase()]??1,g=s>12?Math.min(1.6,1+(s-12)*.022):1;return Math.max(3,Math.min(14,5*d*g))}function be(o){return o==="BUY"?'<span class="badge" style="background:var(--emerald-dim); color:var(--emerald); border:1px solid rgba(16,185,129,0.22)">▲ BUY</span>':o==="SELL"?'<span class="badge" style="background:var(--crimson-dim); color:var(--crimson); border:1px solid rgba(239,68,68,0.22)">▼ SELL</span>':'<span class="badge" style="background:rgba(var(--text-rgb,0,0,0),0.05); color:var(--text-faint); border:1px solid var(--border)">—</span>'}function Ye(o){if(o==null)return'<span class="mono" style="color:var(--text-faint)">—</span>';const s=Number(o),d=s>.5?"var(--emerald)":s<-.5?"var(--crimson)":"var(--text-muted)",g=s>.5?"↑":s<-.5?"↓":"·",L=s>0?"+":"";return`<span class="mono" style="color:${d}; font-weight:700">${g} ${L}${s.toFixed(1)}</span>`}function Ie(o){if(o==null)return'<span class="mono" style="color:var(--text-faint)">—</span>';const s=Number(o),d=s>=12?"var(--emerald)":s<=-12?"var(--crimson)":"var(--text-muted)",g=s>0?"↑":s<0?"↓":"·",L=s>0?"+":"";return`<span class="mono" style="color:${d}; font-weight:700">${g} ${L}${s}</span>`}function Ke(o,s,d){if(s==null)return`<span class="mono" style="color:var(--text-faint); font-size:11px">${o!=null&&o.toFixed?o.toFixed(1):o} <span style="color:var(--text-faint)">· market —</span></span>`;const g=Math.max(Math.abs(o),Math.abs(s),10),L=Math.round(Math.abs(o)/g*100),H=Math.round(Math.abs(s)/g*100),T=d>0?"var(--emerald)":d<0?"var(--crimson)":"var(--text-faint)";return`<div style="display:flex; align-items:center; gap:6px; min-width:160px"><span class="mono" style="font-size:11px; min-width:44px; text-align:right">${o.toFixed(1)}</span><div style="flex:1; height:4px; background:rgba(var(--text-rgb,0,0,0),0.06); border-radius:999px; position:relative; overflow:hidden"><div style="position:absolute; left:0; top:0; bottom:0; width:${L}%; background:var(--amber); opacity:0.9; border-radius:999px"></div><div style="position:absolute; left:0; top:0; bottom:0; width:${H}%; background:var(--sky); opacity:0.35; border-radius:999px"></div></div><span class="mono" style="font-size:11px; color:var(--text-muted); min-width:36px">${s.toFixed(1)}</span><span class="mono" style="font-size:11px; color:${T}; font-weight:700; min-width:36px; text-align:right">${d>0?"+":""}${d.toFixed(1)}</span></div>`}async function X(o){var oe,se;const s=new URLSearchParams(location.hash.split("?")[1]||""),d=s.get("week");d!=null&&d!==""&&Number.isFinite(Number(d))&&(S=Number(d)),u=s.get("q")||((oe=document.getElementById("globalSearch"))==null?void 0:oe.value)||"",l=1;const g=Number(s.get("limit")||800),L=Number.isFinite(g)?Math.max(10,Math.min(2e3,Math.floor(g))):800,H=e=>String(e||"").toLowerCase().replace(/\b(jr\.?|sr\.?|ii|iii|iv|v)\b/g,"").replace(/[^a-z0-9 ]/g,"").replace(/\s+/g," ").trim();let T=new Map;Z=new Set;try{const e=await je({}),a=[].concat(e.starters||[],e.bench||[],e.reserve||[]).map(r=>({player_id:r.player_id,team_name:r.team_name||""}));for(const r of a)if(r.player_id&&(Z.add(String(r.player_id)),r.player_name)){const n=`${H(r.player_name)}|${(r.position||"").toUpperCase()}`;T.set(n,String(r.player_id))}}catch{}const I=b?await ze({limit:L}):await Ee({week:S,limit:L});k=b?(I.players||[]).map(e=>{const a=Math.max(1,Number(e.remaining_games)||1),r=U(e.position,Number(e.per_game_neutral)||0),n=Number((r*Math.sqrt(a)).toFixed(2));return{...e,projected_points:e.ros_points,point_estimate:e.ros_points,player_name:e.player_name||e.player_display_name,position_group:e.position,width:n,projection_lower:Math.max(0,Number((e.ros_points-n).toFixed(2))),projection_upper:Number((e.ros_points+n).toFixed(2))}}):I.players||[];const B=I.meta||{};!b&&S==null&&B.week!=null&&(S=B.week);let R={players:[],count:0,meta:{},fetched_at:null};try{R=await Ne({limit:2e3})}catch{R={players:[],count:0,meta:{},fetched_at:null}}f=new Map((R.players||[]).map(e=>[String(e.player_id),e]));for(const e of k){const a=`${H(e.player_name||"")}|${(e.position||"").toUpperCase()}`;!e.sleeper_id&&T.has(a)&&(e.sleeper_id=T.get(a))}const ee=new Set(k.map(e=>String(e.player_id)));for(const e of R.players||[]){const a=String(e.player_id||"");a&&!ee.has(a)&&(ee.add(a),k.push({player_id:a,sleeper_id:e.sleeper_id||(/^\d+$/.test(a)?a:null),espn_id:e.espn_id||null,player_name:e.player_name||e.full_name||a,position:(e.position||"UNK").toUpperCase(),team:(e.team||"").toUpperCase(),projected_points:e.model_points??e.projected_points??0,point_estimate:e.model_points??e.projected_points??0,projection_lower:e.projection_lower??(e.model_points??0)-U(e.position,e.model_points??0),projection_upper:e.projection_upper??(e.model_points??0)+U(e.position,e.model_points??0),width:e.width??U(e.position,e.model_points??0),injury_status:e.injury_status||null,market_points:e.market_points,delta_points:e.delta_points,model_overall_rank:e.model_overall_rank,model_pos_rank:e.model_pos_rank,fp_ecr:e.fp_ecr,fp_ecr_pos:e.fp_ecr_pos,fp_adp:e.fp_adp,fp_tier:e.fp_tier,delta_rank:e.delta_rank,delta_pos_rank:e.delta_pos_rank,edge:e.edge||"NEUTRAL",edge_score:e.edge_score||0,stat_deltas:e.stat_deltas||[]}))}const m=f.size>0;for(const e of k){e.player_id&&/^\d+$/.test(String(e.player_id))&&(e.sleeper_id=e.player_id);const a=f.get(String(e.player_id));if(a?(a.sleeper_id&&(e.sleeper_id=a.sleeper_id),a.espn_id&&(e.espn_id=a.espn_id),e.market_points=a.market_points,e.delta_points=a.delta_points,e.model_overall_rank=a.model_overall_rank,e.model_pos_rank=a.model_pos_rank,e.fp_ecr=a.fp_ecr,e.fp_ecr_pos=a.fp_ecr_pos,e.fp_adp=a.fp_adp,e.fp_tier=a.fp_tier,e.delta_rank=a.delta_rank,e.delta_pos_rank=a.delta_pos_rank,e.edge=a.edge,e.edge_score=a.edge_score,e.stat_deltas=a.stat_deltas,e.market_season_stats=a.market_season_stats||null,e.auction=a.auction,e.gridironAuction=a.auction,e.marketAuction=a.marketAuction,e.vor=a.vor):(e.market_points=null,e.delta_points=null,e.edge="NEUTRAL",e.stat_deltas=[]),b){e.weekly=Number(e.per_game_neutral)||0,e.lower=e.projection_lower,e.upper=e.projection_upper;const r=a&&a.market_season_points!=null&&Number(a.market_season_points)>0?Number(a.market_season_points):null;e.market_points=r,e.delta_points=r!=null?Number((e.projected_points-r).toFixed(2)):null}else{const r=a&&a.model_points!=null&&Number(a.model_points)>0?Number(a.model_points):null,n=e.projected_points!=null&&Number(e.projected_points)>0?Number(e.projected_points):null,M=a&&a.market_season_points!=null&&Number(a.market_season_points)>0?Number(a.market_season_points)/17:null,w=r??n??M??0;e.projected_points=Number(w.toFixed(2)),e.point_estimate=Number(w.toFixed(2)),e.weekly=Number(w.toFixed(2));const q=(a==null?void 0:a.interval_width)??(a==null?void 0:a.width)??e.width,E=Number(q??U(e.position,w));e.width=Number(E.toFixed(2)),e.projection_lower=Number(Math.max(0,w-E).toFixed(2)),e.projection_upper=Number((w+E).toFixed(2)),e.lower=e.projection_lower,e.upper=e.projection_upper}e.ecr=e.fp_ecr,e.adp=e.fp_adp,e.tier=e.fp_tier}const y=document.getElementById("globalSearch");y&&!y.dataset.bound&&(y.dataset.bound="1",y.addEventListener("input",ge(()=>{u=y.value,l=1,x(),G()},150)),y.addEventListener("keydown",e=>{e.key==="/"&&document.activeElement!==y&&(e.preventDefault(),y.focus())})),y&&(y.value=u);const K=[...f.values()].filter(e=>e.edge==="BUY").length,V=[...f.values()].filter(e=>e.edge==="SELL").length,O=[...f.values()].filter(e=>e.market_points!=null).length,$=O>0||[...f.values()].some(e=>e.fp_ecr!=null||e.fp_adp!=null);$||(i=!1),o.innerHTML=`
    ...
    ${!b&&B.stale?`<div class="alert alert-warn reveal in" role="status" style="margin-top:12px">${C(B.note||`No precomputed projections for week ${S??B.week} — showing nearest available data.`)}</div>`:""}
    ...
    <div class="kpi-row reveal in" style="margin-top:4px">
      <div class="kpi-card" style="border-top:1px solid var(--emerald)">
        ...
        <div class="kpi-value" style="color:var(--emerald)">${K}</div>
        <div class="kpi-bar"><div class="kpi-bar-fill good" style="width:${Math.min(100,Math.round(K/Math.max(1,Math.min(40,f.size/6))*100))}%"></div></div>
      ...
      <div class="kpi-card" style="border-top:1px solid var(--crimson)">
        ...
        <div class="kpi-bar"><div class="kpi-bar-fill bad" style="width:${Math.min(100,Math.round(V/Math.max(1,Math.min(40,f.size/6))*100))}%"></div></div>
      ...
      <div class="kpi-card" style="border-top:1px solid var(--sky)">
        ...
        <div class="kpi-value" style="color:var(--sky)">${O} / ${f.size}</div>
        <div class="kpi-bar"><div class="kpi-bar-fill" style="background:var(--sky); width:${Math.round(O/Math.max(1,f.size)*100)}%"></div></div>
      ...
      <div class="card-body" style="display:flex; flex-wrap:wrap; gap:8px; align-items:center; justify-content:space-between">
        <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center">
          ...
          <div style="display:flex; gap:6px; margin-left:8px; flex-wrap:wrap">
            ...
            <button class="chip ${_==="BUY"?"active":""}" data-edge="BUY" style="${_==="BUY"?"background:var(--emerald-dim); border-color:rgba(16,185,129,0.35); color:var(--emerald)":""}">▲ BUY (${K})</button>
      ...
      <div class="card-body" style="display:flex; flex-direction:column; gap:12px">
        ...
        <div class="row" style="gap:8px">
          ...
          <div class="filters week-picker-scroll" style="overflow-x:auto; flex-wrap:nowrap; max-width:100%; padding-bottom:4px">
            ${Array.from({length:18},(e,a)=>a+1).map(e=>`<button class="chip ${e===Number(S)?"active":""}" data-proj-week="${e}" title="Show week ${e} projections" style="flex-shrink:0">${e}</button>`).join("")}
          </div>
        </div>
        `}
        <div class="row">
  ...
  `;const te=o.querySelector("#toggleCompare");te&&te.addEventListener("click",()=>{i=!i,X(o)});const ae=o.querySelector("#toggleRos");ae&&ae.addEventListener("click",()=>{b=!b,X(o)}),o.querySelectorAll("[data-proj-week]").forEach(e=>{e.addEventListener("click",()=>{const a=Number(e.getAttribute("data-proj-week"));S=a===S?null:a,l=1,X(o)})}),o.querySelectorAll("[data-edge]").forEach(e=>{e.addEventListener("click",()=>{_=e.getAttribute("data-edge"),l=1,x(),o.querySelectorAll("[data-edge]").forEach(a=>{const r=a.getAttribute("data-edge");r===_?(a.classList.add("active"),r==="BUY"?(a.style.background="var(--emerald-dim)",a.style.borderColor="rgba(16,185,129,0.35)",a.style.color="var(--emerald)"):r==="SELL"?(a.style.background="var(--crimson-dim)",a.style.borderColor="rgba(239,68,68,0.35)",a.style.color="var(--crimson)"):(a.style.background="",a.style.borderColor="",a.style.color="")):(a.classList.remove("active"),a.style.background="",a.style.borderColor="",a.style.color="")})})});const A=o.querySelector("#localSearch");A&&(A.value=u,A.addEventListener("input",ge(()=>{u=A.value,l=1,y&&(y.value=u),x(),G()},150))),o.querySelectorAll("[data-chip]").forEach(e=>{e.addEventListener("click",()=>{const a=e.getAttribute("data-chip");u=u.includes(a)?u.replace(a,"").replace(/\s{2,}/g," ").trim():u?`${u} ${a}`:a,l=1,A&&(A.value=u),y&&(y.value=u),x(),G()})});let F="projected_points",j=-1,D=!1;o.querySelectorAll("th[data-sort]").forEach(e=>{e.style.cursor="pointer";const a=()=>{const r=e.getAttribute("data-sort");F===r?j*=-1:(F=r,j=r==="player_name"?1:-1),D=!0,l=1,x(),Q()};e.addEventListener("click",a),e.addEventListener("keydown",r=>{(r.key==="Enter"||r.key===" ")&&(r.preventDefault(),a())})});function Q(){const e=o.querySelector("#toggleProjSortDir");if(!e)return;const a=j===-1?"Highest → Lowest":"Lowest → Highest";e.textContent=`↕ ${a}`,e.title=`Currently ${a} by ${F}: click to flip`}(se=o.querySelector("#toggleProjSortDir"))==null||se.addEventListener("click",()=>{j*=-1,D=!0,l=1,x(),Q()}),Q();function fe(e){for(const r of e)r._onRoster=Z.has(String(r.player_id));let a=Ue(e,u);return m&&_!=="ALL"&&(i||!$)&&(a=a.filter(r=>(r.edge||"NEUTRAL")===_)),a}function re(e,a){let r=e[a];if((r==null||r===""||r==="—")&&a==="projected_points"&&(r=e.point_estimate),r==null||r===""||r==="—"||r==="–"||r==="-")return null;if(typeof r=="number")return isNaN(r)?null:r;const n=String(r).trim(),M=Number(n);return!isNaN(M)&&n!==""?M:n.toLowerCase()}function x(){var ie,de,pe,ce;let e=fe(k);const a=D?null:De(k);e=[...e].sort((t,v)=>{if(!D){const h=ve(t,{},a)-ve(v,{},a);if(h!==0)return h;const z=ye(t,{},a)-ye(v,{},a);if(z!==0)return z}const p=re(t,F),c=re(v,F);return p===null&&c===null?0:p===null?1:c===null?-1:typeof p=="number"&&typeof c=="number"?(p-c)*j:String(p).localeCompare(String(c))*j}),o.querySelectorAll("th[data-sort]").forEach(t=>{t.getAttribute("data-sort")===F?t.setAttribute("aria-sort",j===1?"ascending":"descending"):t.removeAttribute("aria-sort")});const r=e.length,n=Math.max(1,Math.ceil(r/J));l>n&&(l=n);const M=(l-1)*J,w=Math.min(M+J,r),q=e.slice(M,w),E=o.querySelector("#countLabel");E&&(r===0?E.textContent="0 players":E.textContent=`Showing ${M+1}–${w} of ${r} players${r!==k.length?` (filtered from ${k.length})`:""}${m&&_!=="ALL"?` · ${_} only`:""}`);const W=o.querySelector("#projBody"),le=i&&m?15:9;q.length?(W.innerHTML=q.map(t=>{const v=t.position||t.position_group||"UNK",p=Number(t.projected_points??t.point_estimate??0),c=Number(t.projection_lower??t.lower_bound??Math.max(0,p-(t.width??5))),h=Number(t.projection_upper??t.upper_bound??p+(t.width??5)),z=Number(t.width??t.projection_width??(h-c)/2),_e=t.market_points!=null?Number(t.market_points).toFixed(1):"—",he=t.fp_ecr!=null?`#${t.fp_ecr}${t.fp_ecr_pos?` (#${t.fp_ecr_pos} ${v})`:""}${t.fp_tier?` <span style="background:var(--violet-dim); color:var(--violet); border:1px solid rgba(168,85,247,0.18); border-radius:999px; padding:1px 5px; font:700 10px ui-monospace, SFMono-Regular,monospace">T${t.fp_tier}</span>`:""}`:"—",xe=t.fp_adp!=null?`#${t.fp_adp}`:"—",ke=i&&m?be(t.edge):"",Y=Pe(t.team),$e=t.team?`<span class="badge" style="background:${Y}1f; color:${Y}; border:1px solid ${Y}3d; font-weight:700">${me(t.team,14)} ${C(t.team)}</span>`:"—",we=t.edge==="BUY"?"var(--emerald)":t.edge==="SELL"?"var(--crimson)":Y,Se=i&&m?`<button class="chip" data-expand="${t.player_id}" aria-label="Show stat deltas for ${C(t.player_name)}" style="padding:4px 8px; font-size:11px">▶</button>`:"",ue=`
          <tr data-team="${t.team||""}" data-pid="${t.player_id}" class="clickable-row" style="cursor:pointer; --team-accent:${we}; ${t.edge==="BUY"?"background:rgba(16,185,129,0.04)":t.edge==="SELL"?"background:rgba(239,68,68,0.04)":""}">
            <td><div class="player-cell">${Ce(t,32)}<div class="player-cell-info"><div class="player-cell-name">${C(t.player_name||t.player_id)}</div><div class="player-cell-sub">${me(t.team,14)} ${C(t.team||"—")} ${t.model_pos_rank?`<span style="color:var(--text-faint)">· #${t.model_pos_rank} ${v}</span>`:""}</div></div></div></td>
            <td>${Be(v)}</td>
            <td>${$e}</td>
            <td class="mono" style="font-weight:700; color:var(--amber)">${p.toFixed(1)}</td>
            ${i&&m?`
            <td class="mono" style="color:var(--sky)">${_e}</td>
            <td>${Ye(t.delta_points)}</td>
            <td class="mono" style="font-size:11px; color:var(--text-muted)">${he}</td>
            <td>${Ie(t.delta_rank)}</td>
            <td class="mono" style="font-size:11px; color:var(--text-muted)">${xe}</td>
            <td>${ke}</td>
            `:""}
            <td>${Re({point:p,low:c,high:h,width:z,min:0,max:35})}</td>
            <td>${Ae(t.wind_mph)}</td>
            <td>${Fe(z)}</td>
            <td>${Te(t.injury_status)} ${t.trending?'<span class="badge" style="background:var(--sky-dim); color:var(--sky); margin-left:6px">↗ trending</span>':""}</td>
            ${i&&m?`<td>${Se}</td>`:""}
          </tr>
        `;if(i&&m&&t.stat_deltas&&t.stat_deltas.length){const Le=t.stat_deltas.filter(P=>P.market!=null||P.model!=null).slice(0,7).map(P=>`
...
`)}).join("")),o.querySelectorAll("[data-pid]").forEach(t=>{t.classList.contains("expand-panel")||(t.style.cursor="pointer",t.addEventListener("click",v=>{if(v.target.closest("[data-expand]"))return;const p=t.getAttribute("data-pid"),c=k.find(h=>String(h.player_id)===String(p));c&&qe(c,o)}))});const N=o.querySelector("#paginationControls");N&&(n<=1?N.innerHTML="":(N.innerHTML=`
        ...
        `,(ie=N.querySelector("#firstPageBtn"))==null||ie.addEventListener("click",()=>{l>1&&(l=1,x())}),(de=N.querySelector("#prevPageBtn"))==null||de.addEventListener("click",()=>{l>1&&(l--,x())}),(pe=N.querySelector("#nextPageBtn"))==null||pe.addEventListener("click",()=>{l<n&&(l++,x())}),(ce=N.querySelector("#lastPageBtn"))==null||ce.addEventListener("click",()=>{l<n&&(l=n,x())})))}function G(){const e="projections";location.hash=u?`${e}?q=${encodeURIComponent(u)}`:e}x()}function ge(o,s=150){let d;return(...g)=>{clearTimeout(d),d=setTimeout(()=>o(...g),s)}}export{X as renderProjections};
