import{b as Ne,x as Pe,g as Ce,e as j,m as Be,y as Re,t as Z,p as Ae,j as Fe,n as ze,z as Te,A as qe,q as Ue,o as De,B as He,C as We}from"./index-B-4KdT_p.js";import{p as Ye}from"./playerCard-ChAFuOem.js";import{b as Ie,r as ve,a as ye}from"./relevance-C3SYLMeE.js";let k=[],f=new Map,u="",n=1,i=!0,_="ALL",J=new Set,g=!1,w=null;const X=50,Ke={QB:1.55,RB:1.07,WR:1.12,TE:.88,K:.85,DEF:.75};function U(o,s){const d=Ke[(o||"UNK").toUpperCase()]??1,b=s>12?Math.min(1.6,1+(s-12)*.022):1;return Math.max(3,Math.min(14,5*d*b))}function ge(o){return o==="BUY"?'<span class="badge" style="background:var(--emerald-dim); color:var(--emerald); border:1px solid rgba(16,185,129,0.22)">▲ BUY</span>':o==="SELL"?'<span class="badge" style="background:var(--crimson-dim); color:var(--crimson); border:1px solid rgba(239,68,68,0.22)">▼ SELL</span>':'<span class="badge" style="background:rgba(var(--text-rgb,0,0,0),0.05); color:var(--text-faint); border:1px solid var(--border)">—</span>'}function Ve(o){if(o==null)return'<span class="mono" style="color:var(--text-faint)">—</span>';const s=Number(o),d=s>.5?"var(--emerald)":s<-.5?"var(--crimson)":"var(--text-muted)",b=s>.5?"↑":s<-.5?"↓":"·",L=s>0?"+":"";return`<span class="mono" style="color:${d}; font-weight:700">${b} ${L}${s.toFixed(1)}</span>`}function Oe(o){if(o==null)return'<span class="mono" style="color:var(--text-faint)">—</span>';const s=Number(o),d=s>=12?"var(--emerald)":s<=-12?"var(--crimson)":"var(--text-muted)",b=s>0?"↑":s<0?"↓":"·",L=s>0?"+":"";return`<span class="mono" style="color:${d}; font-weight:700">${b} ${L}${s}</span>`}function Qe(o,s,d){if(s==null)return`<span class="mono" style="color:var(--text-faint); font-size:11px">${o!=null&&o.toFixed?o.toFixed(1):o} <span style="color:var(--text-faint)">· market —</span></span>`;const b=Math.max(Math.abs(o),Math.abs(s),10),L=Math.round(Math.abs(o)/b*100),D=Math.round(Math.abs(s)/b*100),z=d>0?"var(--emerald)":d<0?"var(--crimson)":"var(--text-faint)";return`<div style="display:flex; align-items:center; gap:6px; min-width:160px"><span class="mono" style="font-size:11px; min-width:44px; text-align:right">${o.toFixed(1)}</span><div style="flex:1; height:4px; background:rgba(var(--text-rgb,0,0,0),0.06); border-radius:999px; position:relative; overflow:hidden"><div style="position:absolute; left:0; top:0; bottom:0; width:${L}%; background:var(--amber); opacity:0.9; border-radius:999px"></div><div style="position:absolute; left:0; top:0; bottom:0; width:${D}%; background:var(--sky); opacity:0.35; border-radius:999px"></div></div><span class="mono" style="font-size:11px; color:var(--text-muted); min-width:36px">${s.toFixed(1)}</span><span class="mono" style="font-size:11px; color:${z}; font-weight:700; min-width:36px; text-align:right">${d>0?"+":""}${d.toFixed(1)}</span></div>`}async function ee(o){var se,ne;const s=new URLSearchParams(location.hash.split("?")[1]||""),d=s.get("week");d!=null&&d!==""&&Number.isFinite(Number(d))&&(w=Number(d)),u=s.get("q")||((se=document.getElementById("globalSearch"))==null?void 0:se.value)||"",n=1;const b=Number(s.get("limit")||800),L=Number.isFinite(b)?Math.max(10,Math.min(2e3,Math.floor(b))):800,D=e=>String(e||"").toLowerCase().replace(/\b(jr\.?|sr\.?|ii|iii|iv|v)\b/g,"").replace(/[^a-z0-9 ]/g,"").replace(/\s+/g," ").trim();let z=new Map;J=new Set;try{const e=await Ne({}),a=[].concat(e.starters||[],e.bench||[],e.reserve||[]).map(r=>({player_id:r.player_id,team_name:r.team_name||""}));for(const r of a)if(r.player_id&&(J.add(String(r.player_id)),r.player_name)){const l=`${D(r.player_name)}|${(r.position||"").toUpperCase()}`;z.set(l,String(r.player_id))}}catch{}const Y=g?await He({limit:L}):await Pe({week:w,limit:L});k=g?(Y.players||[]).map(e=>{const a=Math.max(1,Number(e.remaining_games)||1),r=U(e.position,Number(e.per_game_neutral)||0),l=Number((r*Math.sqrt(a)).toFixed(2));return{...e,projected_points:e.ros_points,point_estimate:e.ros_points,player_name:e.player_name||e.player_display_name,position_group:e.position,width:l,projection_lower:Math.max(0,Number((e.ros_points-l).toFixed(2))),projection_upper:Number((e.ros_points+l).toFixed(2))}}):Y.players||[];const B=Y.meta||{};!g&&w==null&&B.week!=null&&(w=B.week);let R={players:[],count:0,meta:{},fetched_at:null};try{R=await Ce({limit:2e3})}catch{R={players:[],count:0,meta:{},fetched_at:null}}f=new Map((R.players||[]).map(e=>[String(e.player_id),e]));for(const e of k){const a=`${D(e.player_name||"")}|${(e.position||"").toUpperCase()}`;!e.sleeper_id&&z.has(a)&&(e.sleeper_id=z.get(a))}const te=new Set(k.map(e=>String(e.player_id)));for(const e of R.players||[]){const a=String(e.player_id||"");a&&!te.has(a)&&(te.add(a),k.push({player_id:a,sleeper_id:e.sleeper_id||(/^\d+$/.test(a)?a:null),espn_id:e.espn_id||null,player_name:e.player_name||e.full_name||a,position:(e.position||"UNK").toUpperCase(),team:(e.team||"").toUpperCase(),projected_points:e.model_points??e.projected_points??0,point_estimate:e.model_points??e.projected_points??0,projection_lower:e.projection_lower??(e.model_points??0)-U(e.position,e.model_points??0),projection_upper:e.projection_upper??(e.model_points??0)+U(e.position,e.model_points??0),width:e.width??U(e.position,e.model_points??0),injury_status:e.injury_status||null,market_points:e.market_points,delta_points:e.delta_points,model_overall_rank:e.model_overall_rank,model_pos_rank:e.model_pos_rank,fp_ecr:e.fp_ecr,fp_ecr_pos:e.fp_ecr_pos,fp_adp:e.fp_adp,fp_tier:e.fp_tier,delta_rank:e.delta_rank,delta_pos_rank:e.delta_pos_rank,edge:e.edge||"NEUTRAL",edge_score:e.edge_score||0,stat_deltas:e.stat_deltas||[]}))}const m=f.size>0;for(const e of k){e.player_id&&/^\d+$/.test(String(e.player_id))&&(e.sleeper_id=e.player_id);const a=f.get(String(e.player_id));if(a?(a.sleeper_id&&(e.sleeper_id=a.sleeper_id),a.espn_id&&(e.espn_id=a.espn_id),e.market_points=a.market_points,e.delta_points=a.delta_points,e.model_overall_rank=a.model_overall_rank,e.model_pos_rank=a.model_pos_rank,e.fp_ecr=a.fp_ecr,e.fp_ecr_pos=a.fp_ecr_pos,e.fp_adp=a.fp_adp,e.fp_tier=a.fp_tier,e.delta_rank=a.delta_rank,e.delta_pos_rank=a.delta_pos_rank,e.edge=a.edge,e.edge_score=a.edge_score,e.stat_deltas=a.stat_deltas,e.market_season_stats=a.market_season_stats||null,e.auction=a.auction,e.gridironAuction=a.auction,e.marketAuction=a.marketAuction,e.vor=a.vor):(e.market_points=null,e.delta_points=null,e.edge="NEUTRAL",e.stat_deltas=[]),g){e.weekly=Number(e.per_game_neutral)||0,e.lower=e.projection_lower,e.upper=e.projection_upper;const r=a&&a.market_season_points!=null&&Number(a.market_season_points)>0?Number(a.market_season_points):null;e.market_points=r,e.delta_points=r!=null?Number((e.projected_points-r).toFixed(2)):null}else{const r=a&&a.model_points!=null&&Number(a.model_points)>0?Number(a.model_points):null,l=e.projected_points!=null&&Number(e.projected_points)>0?Number(e.projected_points):null,M=a&&a.market_season_points!=null&&Number(a.market_season_points)>0?Number(a.market_season_points)/17:null,S=r??l??M??0;e.projected_points=Number(S.toFixed(2)),e.point_estimate=Number(S.toFixed(2)),e.weekly=Number(S.toFixed(2));const T=(a==null?void 0:a.interval_width)??(a==null?void 0:a.width)??e.width,N=Number(T??U(e.position,S));e.width=Number(N.toFixed(2)),e.projection_lower=Number(Math.max(0,S-N).toFixed(2)),e.projection_upper=Number((S+N).toFixed(2)),e.lower=e.projection_lower,e.upper=e.projection_upper}e.ecr=e.fp_ecr,e.adp=e.fp_adp,e.tier=e.fp_tier}const y=document.getElementById("globalSearch");y&&!y.dataset.bound&&(y.dataset.bound="1",y.addEventListener("input",be(()=>{u=y.value,n=1,x(),Q()},150)),y.addEventListener("keydown",e=>{e.key==="/"&&document.activeElement!==y&&(e.preventDefault(),y.focus())})),y&&(y.value=u);const I=[...f.values()].filter(e=>e.edge==="BUY").length,K=[...f.values()].filter(e=>e.edge==="SELL").length,V=[...f.values()].filter(e=>e.market_points!=null).length,$=V>0||[...f.values()].some(e=>e.fp_ecr!=null||e.fp_adp!=null);$||(i=!1),o.innerHTML=`
    ...
    ${!g&&B.stale?`<div class="alert alert-warn reveal in" role="status" style="margin-top:12px">${j(B.note||`No precomputed projections for week ${w??B.week} — showing nearest available data.`)}</div>`:""}
      ...
      <table id="projTable" style="min-width:${i&&m?"1180px":"760px"}">
        <thead>
          <tr>
            ...
            <th data-sort="projected_points" tabindex="0" role="button" aria-label="Sort by Model Points" style="${i&&m?"color:var(--amber); border-bottom:2px solid var(--amber)":""}">${g?"RoS":"Model"}<br><span style="font:600 10px "Helvetica Neue", Helvetica,sans-serif; color:${i&&m?"var(--amber)":"var(--text-faint)"}; opacity:0.7">${g?"total":"proj"}</span></th>
            ${i&&m?`
...
`)}).join("")),o.querySelectorAll("[data-pid]").forEach(t=>{t.classList.contains("expand-panel")||(t.style.cursor="pointer",t.addEventListener("click",v=>{if(v.target.closest("[data-expand]"))return;const p=t.getAttribute("data-pid"),c=k.find(h=>String(h.player_id)===String(p));c&&De(c,o,w??void 0)}))});const P=o.querySelector("#paginationControls");P&&(l<=1?P.innerHTML="":(P.innerHTML=`
        ...
        `,(de=P.querySelector("#firstPageBtn"))==null||de.addEventListener("click",()=>{n>1&&(n=1,x())}),(pe=P.querySelector("#prevPageBtn"))==null||pe.addEventListener("click",()=>{n>1&&(n--,x())}),(ce=P.querySelector("#nextPageBtn"))==null||ce.addEventListener("click",()=>{n<l&&(n++,x())}),(ue=P.querySelector("#lastPageBtn"))==null||ue.addEventListener("click",()=>{n<l&&(n=l,x())})))}function Q(){const e="projections";location.hash=u?`${e}?q=${encodeURIComponent(u)}`:e}x()}function be(o,s=150){let d;return(...b)=>{clearTimeout(d),d=setTimeout(()=>o(...b),s)}}export{ee as renderProjections};
