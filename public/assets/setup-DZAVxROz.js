import{f as q,E as C,i as k,e as i,v as B,M as L,N as w,O as I,k as H,P as R,Q as _,R as U,S as T,T as A}from"./index-DjgPke4c.js";function j(n){return`<div class="player-modal-backdrop show" id="setupBackdrop" style="position:fixed; inset:0; z-index:2000; background:rgba(0,0,0,0.55); display:flex; align-items:center; justify-content:center; padding:16px">
  ...
  </div>`}function O(n){return n==="auction"?'<span class="badge" style="background:var(--amber-dim); color:var(--amber)">auction</span>':n==="snake"?'<span class="badge" style="background:var(--sky-dim); color:var(--sky)">snake</span>':'<span class="badge">draft type unknown</span>'}async function N({onDone:n}={}){const d=document.activeElement;let e=document.getElementById("setupModalRoot");e||(e=document.createElement("div"),e.id="setupModalRoot",document.body.appendChild(e));const b=t=>{e.innerHTML="";try{d&&d.focus&&d.focus()}catch{}typeof n=="function"&&n(t)},v=await q().catch(()=>null),f=v&&v.configuredLeagues||[],m=C(),E=f.length?f.map(t=>{const a=String(t.league_id||"");if(!a)return"";const s=a===m?"selected":"",c=`${t.league_name||"League"} (${a.slice(0,6)}…)`;return`<option value="${k(a)}" ${s}>${i(c)}</option>`}).join(""):"";e.innerHTML=j(`
    ...
    <div class="row" style="gap:8px; margin-bottom:16px">
      <select id="setupExisting" class="team-select-dropdown" style="flex:1">${E}</select>
  ...
  `);const $=e.querySelector("#setupCard"),M=B($,d,()=>b(!1)),p=e.querySelector("#setupResult"),r=()=>{try{M()}catch{}},o=b;e.querySelector("#setupClose").addEventListener("click",()=>{r(),o(!1)}),e.querySelector("#setupBackdrop").addEventListener("click",t=>{t.target.id==="setupBackdrop"&&(r(),o(!1))});const h=e.querySelector("#setupUseExisting");h&&h.addEventListener("click",()=>{const t=e.querySelector("#setupExisting").value;L(t)&&(w(t),r(),o(!0))}),e.querySelector("#setupValidate").addEventListener("click",async()=>{const t=e.querySelector("#setupInput").value,a=I(t);if(!L(a)){p.innerHTML=`<div class="alert alert-bad">That doesn't look like a Sleeper league ID — paste the digits from your league URL (sleeper.app/leagues/…).</div>`;return}p.innerHTML=`<div class="faint">Looking up league ${i(a)} on Sleeper…</div>`;const s=await H(a);if(!s||!s.league_id){p.innerHTML=`<div class="alert alert-bad">Couldn't reach Sleeper for that ID. Check the digits and your connection, then try again.</div>`;return}const c=R();p.innerHTML=`
        ...
        <div id="setupDataCheck"></div>
      </div>`,e.querySelector("#setupSave").addEventListener("click",async()=>{const D=e.querySelector("#setupDraftType").value;_(D),w(a),U(a,s.league_name,s.season);const x=e.querySelector("#setupDataCheck");if(x.innerHTML='<div class="faint" style="margin-top:8px">Checking synced data…</div>',await T().catch(()=>null)){r(),o(!0);return}(l=>{x.innerHTML=`
            <div class="alert alert-info" style="margin-top:12px">
              ...
              <div id="syncProgressArea" style="margin-top:10px">${l}</div>
                  ...
                  <button class="btn btn-ghost btn-sm" id="setupDoneBtn" style="margin-left:8px">Done (sync later)</button>`,S();const g=e.querySelector("#setupDoneBtn");g&&g.addEventListener("click",()=>{r(),o(!0)})}}})};S()})})}export{N as openSetupModal};
