import{w as te,f as H,b as ae,k as O,e as se,m as ne,t as U,h as u,x as re,u as W,o as Y,y as oe,q as ie,r as D,n as I,p as V,i as K,j as Q,v as X}from"./index-D0XiqyTN.js";import{c as le,a as de}from"./slots-CAjbNQjp.js";import"./auctionMath-DsCIWbys.js";async function fe(t){const e=new URLSearchParams(location.hash.split("?")[1]||""),v=e.get("week")?Number(e.get("week")):null,[y,l]=await Promise.all([te({week:v}),H().catch(()=>({}))]),h=y.week??v??"",g=y.leagueMatchups||[],x=y.nflSlate||[],s=typeof(l==null?void 0:l.data_source)=="string"&&l.data_source.toLowerCase()==="demo",i=typeof(l==null?void 0:l.weather_status)=="string"&&l.weather_status.toLowerCase()==="placeholder"||(l==null?void 0:l.weather_placeholder)===!0,n=new Map;g.forEach(a=>{const o=a.matchup_id==null?null:String(a.matchup_id);o!=null&&(n.has(o)||n.set(o,[]),n.get(o).push(a))});let r=[],c=new Map,w=new Map;if(n.size>0){const a=await ae({week:v}).catch(()=>null),o=(a==null?void 0:a.rosters)||(a==null?void 0:a.teams)||null,d=a||await O({roster_id:"1"});r=d.leagueRosters||d.allTeams||[];const[f,m]=await Promise.all([(async()=>o&&typeof o=="object"?r.map(p=>{const _=String(p.roster_id),$=o[_]||o[Number(_)]||null;return $?{starters:$.starters||[],bench:$.bench||[],reserve:$.reserve||[],myRoster:$.starters||[],teamMeta:$.team_info||$.teamMeta||p,team_info:$.team_info||$.teamMeta||p}:null}):Promise.all(r.map(p=>O({roster_id:p.roster_id}).catch(()=>null))))(),se({limit:800}).catch(()=>({players:[]}))]);(m.players||[]).forEach(p=>{p.player_id&&w.set(String(p.player_id),p),p.player_name&&w.set(p.player_name.toLowerCase(),p)});const b=await ne(H,ie).catch(()=>null),F={vbdParams:le(m.players||[],b),compPlayers:m.players||[]};f.forEach((p,_)=>{var q;if(!p)return;const $=p.teamMeta||p.team_info||r[_]||{},z=String($.roster_id||((q=r[_])==null?void 0:q.roster_id)||""),J=p.starters||p.myRoster||[],Z=p.bench||[],{starters:C,bench:R}=de(J,Z,F),A=C.reduce((M,P)=>M+P.weekly,0),B=[...C,...R].reduce((M,P)=>M+(P.modelAuction??0),0),N=[...C,...R].map(M=>M.marketAuction),ee=N.every(M=>M!=null)?N.reduce((M,P)=>M+P,0):null;c.set(z,{roster_id:z,owner_name:$.display_name||$.owner_name||`Team ${z}`,team_name:$.team_name||$.display_name||`Team ${z}`,avatar_url:$.avatar_url||null,starters:C,bench:R,starterFPTS:A,totalModel:B,totalMarket:ee})})}const j=[...c.values()].some(a=>[...a.starters,...a.bench].some(o=>o.marketAuction!=null)),k=new Map;x.forEach(a=>{k.set((a.home_team||a.home||"").toUpperCase(),a),k.set((a.away_team||a.away||"").toUpperCase(),a)});const T=`
  ...
  `,S=[...n.entries()].sort((a,o)=>a[0]-o[0]);t.innerHTML=`
    ...
    <div style="display:flex; flex-direction:column; gap:16px; margin-top:12px">
      ${S.length>0?S.map(([a,o])=>{var m,b;const d=c.get(String((m=o[0])==null?void 0:m.roster_id))||null,f=o[1]&&c.get(String((b=o[1])==null?void 0:b.roster_id))||null;return ce(d,f,a,o,k,j)}).join(""):`<div class="card reveal in"><div class="empty">No matchups loaded for week ${h||"—"}. Start the backend server and refresh data.</div></div>`}
    </div>
  ...
  `,t.querySelectorAll("[data-week]").forEach(a=>{a.addEventListener("click",()=>{const o=a.getAttribute("data-week");location.hash=`matchups${o?`?week=${o}`:""}`})}),t.querySelectorAll("[data-matchup-id]").forEach(a=>{const o=()=>{var F,p;const d=String(a.getAttribute("data-matchup-id")),f=S.find(([_])=>String(_)===d);if(!f)return;const[,m]=f,b=c.get(String((F=m[0])==null?void 0:F.roster_id))||null,E=m[1]&&c.get(String((p=m[1])==null?void 0:p.roster_id))||null;b&&pe(b,E,d,k,t,c)};a.addEventListener("click",d=>{d.target.closest("[data-player-id]")||o()}),a.addEventListener("keydown",d=>{if(d.key==="Enter"||d.key===" "){if(d.target.closest&&d.target.closest("[data-player-id]"))return;d.preventDefault(),o()}})}),t.querySelectorAll("[data-player-id]").forEach(a=>{const o=()=>{const d=a.getAttribute("data-player-id");let f=null;c.forEach(m=>{const b=[...m.starters,...m.bench].find(E=>String(E.player_id)===d);b&&(f=b)}),f&&Y(f,t)};a.addEventListener("click",d=>{d.stopPropagation(),o()}),a.addEventListener("keydown",d=>{(d.key==="Enter"||d.key===" ")&&(d.preventDefault(),d.stopPropagation(),o())})})}function ce(t,e,v,y,l,h=!0){if(!t)return`<div class="card reveal in"><div class="empty">Matchup ${v}: Missing roster data</div></div>`;const g=t.starterFPTS,x=e?e.starterFPTS:0,s=Math.sqrt(t.starters.reduce((c,w)=>c+w.width*w.width,0)+(e?e.starters.reduce((c,w)=>c+w.width*w.width,0):0))||10,i=g-x,n=e?Math.round(100*G(i/s)):100,r=100-n;return`
        ...
        <div style="display:grid; grid-template-columns:1fr auto 1fr; align-items:center; padding:16px; gap:12px">
          ...
          <div style="text-align:center; min-width:160px">
            <div style="display:flex; align-items:baseline; justify-content:center; gap:12px">
              <span class="mono" style="font-size:22px; font-weight:800; color:var(--amber)">${g.toFixed(1)}</span>
  ...
  `}function pe(t,e,v,y,l,h){var a,o,d;let g=document.getElementById("matchupModalContainer");g||(g=document.createElement("div"),g.id="matchupModalContainer",document.body.appendChild(g));const x=t.starterFPTS,s=e?e.starterFPTS:0,i=Math.sqrt(t.starters.reduce((f,m)=>f+m.width*m.width,0)+(e?e.starters.reduce((f,m)=>f+m.width*m.width,0):0))||10,n=x-s,r=e?Math.round(100*G(n/i)):100,c=100-r,w=ve(t,e,y),j=t.totalMarket!=null||(e==null?void 0:e.totalMarket)!=null;g.innerHTML=`
  ...
  `;const k=document.activeElement instanceof HTMLElement?document.activeElement:null;let T=()=>{};const S=()=>{try{T()}catch{}g.innerHTML="",k&&typeof k.focus=="function"&&document.contains(k)&&(!document.activeElement||document.activeElement===document.body)&&k.focus()};T=oe(g.querySelector(".matchup-modal-card"),k,S),(a=document.getElementById("matchupCloseBtn"))==null||a.addEventListener("click",S),(o=document.getElementById("matchupDismissBtn"))==null||o.addEventListener("click",S),(d=document.getElementById("matchupModalBackdrop"))==null||d.addEventListener("click",f=>{f.target.id==="matchupModalBackdrop"&&S()}),g.querySelectorAll("[data-player-id]").forEach(f=>{const m=()=>{const b=f.getAttribute("data-player-id");let E=null;h.forEach(F=>{const p=[...F.starters,...F.bench].find(_=>String(_.player_id)===b);p&&(E=p)}),E&&Y(E,l)};f.addEventListener("click",b=>{b.stopPropagation(),m()}),f.addEventListener("keydown",b=>{(b.key==="Enter"||b.key===" ")&&(b.preventDefault(),b.stopPropagation(),m())})})}function L(t,e){if(t==null)return 0;if(t.wind_mph>0)return t.wind_mph;const v=e&&e.get((t.team||"").toUpperCase());return v&&v.wind_mph||0}function ue(t,e,v,y){const l=["QB","RB1","RB2","WR1","WR2","TE","FLEX1","FLEX2","K","DEF"],h=new Map(t.starters.map(s=>[s.slot,s])),g=new Map(((e==null?void 0:e.starters)||[]).map(s=>[s.slot,s])),x=[...new Set([...t.starters.map(s=>s.slot),...((e==null?void 0:e.starters)||[]).map(s=>s.slot)])].filter(s=>!l.includes(s));return`
    <!-- Slot-by-Slot Breakdown -->
    <div style="border-top:1px solid var(--border)">
      <div class="table-wrap matchup-table-scroll" style="border:0; border-radius:0; overflow-x:auto; max-width:100%">
        <table style="min-width:900px" aria-label="Slot-by-slot matchup">
          ...
          <tbody>
            ${[...l,...x].map((s,i)=>{const n=h.get(s)||null,r=g.get(s)||null;return!n&&!r?"":ye(n,r,s,i,y)}).join("")}
    ...
    `:""}
  `}function ye(t,e,v,y,l){t!=null&&t.slot||e!=null&&e.slot||v||y!=null&&y+1;const h=((t==null?void 0:t.weekly)||0)-((e==null?void 0:e.weekly)||0),g=h>2?"color:var(--emerald)":h<-2?"color:var(--crimson)":"color:var(--text-muted)",x=t?D((t.team||"").toUpperCase()):"transparent";return e&&D((e.team||"").toUpperCase()),`
    <tr style="--team-accent:${x}">
      ...
      <td style="text-align:center">
        <span class="mono" style="font-weight:700; ${g}; font-size:12px">
          ${h>0?"+":""}${h.toFixed(1)}
        </span>
      </td>
      <td style="text-align:center">
