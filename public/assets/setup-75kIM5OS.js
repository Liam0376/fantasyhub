import{f as q,H as C,n as k,h as i,y as B,O as L,P as w,Q as H,q as I,R,S as _,T as U,U as T,V as A}from"./index-D0XiqyTN.js";function j(n){return`<div class="player-modal-backdrop show" id="setupBackdrop" style="position:fixed; inset:0; z-index:2000; background:rgba(0,0,0,0.55); display:flex; align-items:center; justify-content:center; padding:16px">
  ...
  </div>`}function O(n){return n==="auction"?'<span class="badge" style="background:var(--amber-dim); color:var(--amber)">auction</span>':n==="snake"?'<span class="badge" style="background:var(--sky-dim); color:var(--sky)">snake</span>':'<span class="badge">draft type unknown</span>'}async function K({onDone:n}={}){const d=document.activeElement;let e=document.getElementById("setupModalRoot");e||(e=document.createElement("div"),e.id="setupModalRoot",document.body.appendChild(e));const b=t=>{e.innerHTML="";try{d&&d.focus&&d.focus()}catch{}typeof n=="function"&&n(t)},m=await q().catch(()=>null),f=m&&m.configuredLeagues||[],v=C(),$=f.length?f.map(t=>{const a=String(t.league_id||"");if(!a)return"";const s=a===v?"selected":"",c=`${t.league_name||"League"} (${a.slice(0,6)}…)`;return`<option value="${k(a)}" ${s}>${i(c)}</option>`}).join(""):"";e.innerHTML=j(`
    ...
    <div class="row" style="gap:8px; margin-bottom:12px">
      <input id="setupInput" class="search-top" style="flex:1; border:1px solid var(--border); border-radius:8px; padding:8px 10px" placeholder="e.g. 123456789012345678" inputmode="numeric" value="${k(v)}" />
  ...
  `);const E=e.querySelector("#setupCard"),D=B(E,d,()=>b(!1)),p=e.querySelector("#setupResult"),r=()=>{try{D()}catch{}},o=b;e.querySelector("#setupClose").addEventListener("click",()=>{r(),o(!1)}),e.querySelector("#setupBackdrop").addEventListener("click",t=>{t.target.id==="setupBackdrop"&&(r(),o(!1))});const h=e.querySelector("#setupUseExisting");h&&h.addEventListener("click",()=>{const t=e.querySelector("#setupExisting").value;L(t)&&(w(t),r(),o(!0))}),e.querySelector("#setupValidate").addEventListener("click",async()=>{const t=e.querySelector("#setupInput").value,a=H(t);if(!L(a)){p.innerHTML=`<div class="alert alert-bad">That doesn't look like a Sleeper league ID — paste the digits from your league URL (sleeper.app/leagues/…).</div>`;return}p.innerHTML=`<div class="faint">Looking up league ${i(a)} on Sleeper…</div>`;const s=await I(a);if(!s||!s.league_id){p.innerHTML=`<div class="alert alert-bad">Couldn't reach Sleeper for that ID. Check the digits and your connection, then try again.</div>`;return}const c=R();p.innerHTML=`
      <div class="card" style="padding:12px; margin-top:4px">
        <div style="font-weight:700">${i(s.league_name||"Unnamed league")}</div>
        <div class="faint mono" style="font-size:11px; margin:4px 0 8px">${i(String(s.total_rosters||"?"))} teams · season ${i(String(s.season||"?"))} ${O(s.draft_type)}</div>
        ...
        <div id="setupDataCheck"></div>
      </div>`,e.querySelector("#setupSave").addEventListener("click",async()=>{const M=e.querySelector("#setupDraftType").value;_(M),w(a),U(a,s.league_name,s.season);const x=e.querySelector("#setupDataCheck");if(x.innerHTML='<div class="faint" style="margin-top:8px">Checking synced data…</div>',await T().catch(()=>null)){r(),o(!0);return}(l=>{x.innerHTML=`
            <div class="alert alert-info" style="margin-top:12px">
              ...
              <div id="syncProgressArea" style="margin-top:10px">${l}</div>
                  ...
                  <button class="btn btn-ghost btn-sm" id="setupDoneBtn" style="margin-left:8px">Done (sync later)</button>`,S();const g=e.querySelector("#setupDoneBtn");g&&g.addEventListener("click",()=>{r(),o(!0)})}}})};S()})})}export{K as openSetupModal};
